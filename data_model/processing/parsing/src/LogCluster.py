...
class LogParser:
    def __init__(
        ...
        for idx, para in enumerate(self.paras):
            if para:
                self.perl_command += " -{} {}".format(
                    self.paranames[idx], para
                )
    ...
    def parse(self, filename):
        ...
        self.df_log = self.log_to_dataframe(
            filepath, regex, headers, self.log_format
        )
    ...
    def wirteResultToFile(self):
        ...
        for i in range(self.df_log.shape[0]):
            ...
            if e_idx != -1:
                ...
                EventId.append(
                    hashlib.md5(content.encode("utf-8")).hexdigest()[0:8]
                )
        ...
        self.df_log.to_csv(
            os.path.join(self.savepath, self.filename + "_structured.csv"),
            index=False,
        )
